<template>
  <div class="comment-part">
    <div class="c-content" ref="contentRef">
      <comment-item
    ...
    <div class="c-input">
      ...
      <button
        ...
        :class="{
          ...
          'send-button-disable': !inputVal.length,
        }"
        @click="handleSend"
      >
...
<script lang="ts" setup>
  ...
  watch(
    () => props.commentList,
    (val) => {
      clonedCommentList.value = JSON.parse(JSON.stringify(val));
  ...
  const generateSortedComments = (clist: Comment[]) => {
    const clonedList: Comment[] = JSON.parse(JSON.stringify(clist));
    const list: Comment[] = [];
  ...
  const handleSend = () => {
    ...
    loading.value = true;
    const replyId = replied.value?._id;
    window.setTimeout(() => {
      const cId = Math.random() + ''; // 提交后生成的评论_id
      const newComment = {
        _id: cId,
        id: cId,
        ownerId: storeUserId ?? '',
      ...
      const newList = [...clonedCommentList.value, newComment];
      clonedCommentList.value = newList;
      comments.value = generateSortedComments(newList);
      loading.value = false;
